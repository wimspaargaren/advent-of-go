...
func solve() (int, int) {
	...
	for _, slice := range input {
		...
		for i := 0; i < len(slice); i++ {
			subSlice := aoc.RemoveElementWithCopy(slice, i)
			if isSafe(subSlice) {
				total2++
				break
			}
...
func isSafe(slice []int) bool {
	return isDecreasingSafe(slice) || isIncreasingSafe(slice)
}

func isIncreasingSafe(x []int) bool {
	for i := 0; i < len(x)-1; i++ {
		diff := aoc.Abs(x[i], x[i+1])
		if x[i] < x[i+1] && diff >= 1 && diff <= 3 {
			continue
		}
		return false
	}
...
func isDecreasingSafe(x []int) bool {
	for i := 0; i < len(x)-1; i++ {
		diff := math.Abs(float64(x[i] - x[i+1]))
		if x[i] > x[i+1] && diff >= 1 && diff <= 3 {
			continue
		}
		return false
